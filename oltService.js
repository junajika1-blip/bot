...
// ==========================================
// 3. Hioso (Puppeteer) - FIXED DOUBLE LOGIN untuk SEMUA MODE
// ==========================================
async function cekRedamanHioso(oltConfig, mac) {
    ...
    executablePath: '/usr/bin/chromium-browser', // Pakai Chromium system
    headless: 'new',
    ...
    try {
        ...
        // ✅ STEP 2: CEK DOUBLE LOGIN (Web Form) - FIX untuk Cibarola & 8Pon
        // Logika ini sekarang dijalankan untuk SEMUA Hioso sebelum masuk ke mode iframe/direct
        if (await page.$('#a')) {
            console.log(`   🔑 Terdeteksi form login web (Double Auth). Mengisi...`);
        ...
        // ==========================================
        if (oltConfig.iframe) {
            console.log(`   Mode: Iframe (Cari leftFrame & mainFrame)`);
        ...
        // ==========================================
            ...
            console.log(`   Mode: Direct URL (Akses halaman ONU)`);
...
async function cekDenganRetry(checkerFn, oltConfig, mac) {
    ...
    console.log(`   ⛔ [${oltConfig.label}] Tetap gagal setelah ${MAX_RETRY_PER_OLT + 1}x percobaan (${lastError}). Lanjut ke OLT berikutnya.`);
    return null;
}
